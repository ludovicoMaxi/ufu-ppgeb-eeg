...
public class EpochServiceImpl implements EpochService {
    ...
    public Epoch findById( Long id ) {
        ...
        return epochRepository.findById( id ).orElseThrow( () -> new ResourceNotFoundException( "Epoch", id ) );
    }
    ...
    public List< Epoch > updateList( EpochList epochList ) {
        ...
        if ( CollectionUtils.isNotEmpty( currentEpochs ) ) {
            ...
            for ( Epoch epoch : currentEpochs ) {
                ...
                if ( nonNull( epoch.getExamId() ) && !epoch.getExamId().equals( epochList.getExamId() ) ) {
                    throw new IllegalArgumentException( epoch + " is not same examId in update=" + epochList.getExamId() );
                }
                ...
                if ( nonNull( epoch.getId() ) ) {
                    ...
                    if ( isNull( oldEpoch ) ) {
                        throw new IllegalArgumentException( "Epoch with id=" + epoch.getId() + " not exist by examID=" + epochList.getExamId() );
                    }
        ...
        log.info( "Épocas do exame atualizadas; examId={}, quantidade={}", epochList.getExamId(), savedEpochs.size() );
        return savedEpochs;
    }
}
