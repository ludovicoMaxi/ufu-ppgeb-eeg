...
public class ExamRequestServiceImpl
  ...
  public ExamRequest save(ExamRequest examRequest) {
    ...
    Assert.notNull(
        examRequest, "examRequest cannot be null.");
    ...
    ExamRequest saved = examRequestRepository.save(
        examRequest);
    log.info(
        "Solicitação de exame criada com id={}",
        saved.getId());
    return saved;
  }

  private void validateExamRequest(
      ExamRequest examRequest) {

    Assert.notNull(
        examRequest, "ExamRequest cannot be null.");
    Assert.notNull(examRequest.getMedicalRecord(),
        "medicalRecord cannot be empty.");
    Assert.notNull(examRequest.getMedicalRequest(),
        "medicalRequest cannot be empty.");
    Assert.hasText(examRequest.getSector(),
        "sector cannot be empty.");
    Assert.hasText(examRequest.getDoctorRequestant(),
        "doctorRequestant cannot be empty.");
    Assert.hasText(examRequest.getUser(),
        "user cannot be empty.");
    Assert.notNull(examRequest.getRequestDate(),
        "requestDate cannot be empty.");
    Assert.notNull(examRequest.getPatient(),
        "patient cannot be null.");
    Assert.notNull(examRequest.getPatient().getId(),
  ...
  public List<ExamRequest> findByFilter(
    ...
    return examRequestRepository.findByFilter(
  ...
  public void delete(Long id) {
    ...
    log.info(
        "Solicitação de exame removida com id={}", id);
  }
  ...
  public ExamRequest update(ExamRequest examRequest) {
    ...
    if (!oldExamRequest.equals(examRequest)) {
      ...
      oldExamRequest.setMedicalRecord(
          examRequest.getMedicalRecord());
      oldExamRequest.setMedicalRequest(
          examRequest.getMedicalRequest());
      oldExamRequest.setAchievementDate(
          examRequest.getAchievementDate());
      oldExamRequest.setAgreement(
          examRequest.getAgreement());
      oldExamRequest.setCityOrigin(
          examRequest.getCityOrigin());
      oldExamRequest.setClinicOrigin(
          examRequest.getClinicOrigin());
      oldExamRequest.setDoctorRequestant(
          examRequest.getDoctorRequestant());
      oldExamRequest.setRequestDate(
          examRequest.getRequestDate());
      oldExamRequest.setSector(
          examRequest.getSector());
      oldExamRequest.setUser(examRequest.getUser());

      examRequest =
          examRequestRepository.save(oldExamRequest);
      log.info(
          "Solicitação de exame atualizada com id={}",
          examRequestId);
    }
